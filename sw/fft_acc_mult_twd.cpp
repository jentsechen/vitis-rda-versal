...
fft_acc_mult_twd(xrt::device &device, const xrt::uuid &uuid,
  ...
  for (int iter = 0; iter < n_iter / (n_batch * n_paral); iter++) {
    ...
    auto row_fft_mm2s_1_rhdl = row_fft_mm2s_1(row_fft_in_buf, iter * 2 + 256);
    auto row_fft_mm2s_2_rhdl = row_fft_mm2s_2(row_fft_in_buf, iter * 2 + 512);
    auto row_fft_mm2s_3_rhdl = row_fft_mm2s_3(row_fft_in_buf, iter * 2 + 768);
